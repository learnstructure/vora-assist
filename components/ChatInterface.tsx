...
const ChatInterface: React.FC<ChatInterfaceProps> = ({ 
  ...
  const performSemanticRetrieval = async (query: string): Promise<DocumentChunk[]> => {
    ...
    const scoredChunks = cachedChunks.map(chunk => ({
      chunk,
      score: geminiService.cosineSimilarity(queryEmbedding, chunk.embedding)
    }));
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const errorMessage: Message = {
        id: (Date.now() + 1).toString(),
        role: 'model',
        content: `I encountered an error: ${err.message || "Unknown error"}. Please check your API configuration.`,
        timestamp: Date.now(),
      };
      ...
      <div className="h-14 lg:h-16 border-b border-zinc-900 flex items-center px-4 lg:px-8 bg-zinc-950/80 backdrop-blur-xl z-20 sticky top-0">
        ...
        <div className="ml-auto flex items-center gap-2 lg:gap-4">
          ...
          <span className={`text-[9px] font-black uppercase tracking-widest px-3 py-1 rounded-xl border ${
            provider === 'gemini' ? 'text-blue-400 border-blue-500/20 bg-blue-500/5' : 'text-orange-400 border-orange-500/20 bg-orange-500/5'
          }`}>
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto px-4 py-6 lg:px-20 lg:py-12 space-y-10 pb-32 scroll-smooth">
        {messages.length === 0 && (
          <div className="h-full flex flex-col items-center justify-center text-center max-w-sm mx-auto space-y-8 animate-fade-in">
            <div className={`w-24 h-24 rounded-[2.5rem] flex items-center justify-center shadow-2xl transition-all duration-700 transform hover:rotate-6 ${
              provider === 'gemini' ? 'bg-blue-600 shadow-blue-500/20' : 'bg-orange-600 shadow-orange-500/20'
            }`}>
            ...
            <div className="space-y-4">
              ...
              <p className="text-zinc-500 text-sm font-medium leading-relaxed">
                I'm VORA Assist. I've indexed your context as {profile.role || 'a professional'}. Use me to query your {documents.length} local documents.
              </p>
            </div>
          </div>
        )}
        ...
        {messages.map((msg) => (
          <div key={msg.id} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
            <div className={`max-w-[95%] sm:max-w-[85%] lg:max-w-[75%] ${msg.role === 'user' ? 'text-right' : 'text-left'}`}>
              <div className={`inline-block px-6 py-4 rounded-[1.8rem] text-sm lg:text-[15px] leading-relaxed transition-all shadow-sm ${
                msg.role === 'user' 
      ...
      <div className="p-4 lg:p-12 bg-gradient-to-t from-zinc-950 via-zinc-950 to-transparent">
        <div className="max-w-4xl mx-auto relative">
          <input
            ...
            placeholder={`Ask VORA Assist everything...`}
          ...
          <button 
            ...
            className={`absolute right-4 top-4 bottom-4 w-12 lg:w-16 transition-all rounded-2xl flex items-center justify-center text-white shadow-2xl ${
              provider === 'gemini' ? 'bg-blue-600 hover:bg-blue-500' : 'bg-orange-600 hover:bg-orange-500'
            } disabled:opacity-5`}
          >
